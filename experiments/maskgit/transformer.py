...
class FeedForward(nn.Module):
    def __init__(self, dim, h_dim, multiple_of=256, bias=False, dropout=0.0):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(self, embed_dim, num_heads, dropout=0.0, use_flash=True, bias=False):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if self.flash:
            ...
            output = F.scaled_dot_product_attention(
                xq, xk, xv, mask, dropout_p=self.dropout if self.training else 0.0
            )
        else:
...
class Block(nn.Module):
    ...
    def forward(self, x, cond, mask=None):
        ...
        x = x + alpha1.unsqueeze(1) * self.attn(
            modulate(self.ln1(x), gamma1, beta1), mask=mask
        )
...
class Transformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        )  # Embedding layer for the 'visual' token
        ...
        if self.proj > 1:
            self.in_proj = nn.Conv2d(
                hidden_dim, hidden_dim, kernel_size=2, stride=2, bias=False
            )
    ...
    def forward(self, x, y, drop_label, mask=None):
        ...
        if self.proj > 1:
            x = rearrange(
                x, "b (h w) c -> b c h w", h=h, w=w, b=b, c=self.hidden_dim
            ).contiguous()
            x = self.in_proj(x)
            _, _, h, w = x.shape
            x = rearrange(
                x,
                "b c h proj_w -> b (h proj_w) c",
                proj_h=h,
                proj_w=w,
                b=b,
                c=self.hidden_dim,
