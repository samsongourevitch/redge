#!/usr/bin/env python3
"""Run Sudoku guidance without MDM priors for one sampler."""
...
from local_paths import DATA_DIR, REPO_DIR
...
def soft_penalty(samples, row_idx, col_idx):
...
def main(config: DictConfig) -> None:
    ...
    subset = dataset.shuffle(seed=int(config.seed)).select(
        range(min(max_samples, len(dataset)))
    )
    eval_loader = DataLoader(
        SimpleSudokuDataset(subset),
    ...
    penalty_fn = partial(soft_penalty, row_idx=row_idxs, col_idx=col_idxs)
    ...
    for batch in pbar:
        ...
        current_solved_rate = total_correct / max(total_samples, 1)
        pbar.set_postfix(
    ...
    solved_rate = total_correct / max(total_samples, 1)
